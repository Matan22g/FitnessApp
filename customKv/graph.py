...
class Graph(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        with self._fbo:
            ...
            self._background_rect = RoundedRectangle(size=self.size, radius=[20, ], source='resources/loginback1.png')
        ...
        with self.canvas:
            ...
            self._fbo_rect = Rectangle(
                size=self.size, texture=self._fbo.texture, source='resources/loginback1.png')
...
class BarPlot(Plot):
    ...
    def draw(self, *args):
        ...
        for k in range(point_len):
            p = points[k]
            ### edit for centering the bar
            x1 = x_px(p[0]) - 0.5 * bar_width
            x2 = x1 + bar_width
            y1 = ymin
            y2 = y_px(p[1])
